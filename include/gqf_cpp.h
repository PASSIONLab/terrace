...
class CQF {
	public:
		...
		class Iterator {
			public:
				Iterator(QFi it, uint64_t key = 0);
		...
		Iterator begin(const key_obj& k = key_obj()) const;
...
template <class key_obj>
CQF<key_obj>::Iterator::Iterator(QFi it, uint64_t key)
	: iter(it), key(key) {};
...
bool CQF<key_obj>::Iterator::done(void) const {
	uint64_t key = 0, value = 0, count = 0;
	qfi_get_key(&iter, &key, &value, &count);
	return (this->key != 0 && this->key < key) || qfi_end(&iter);
}

template<class key_obj>
typename CQF<key_obj>::Iterator CQF<key_obj>::begin(const key_obj& k) const {
	QFi qfi;
	if (k.key == 0)
		qf_iterator_from_position(&this->cqf, &qfi, 0);
	else
		qf_iterator_from_key_value(&this->cqf, &qfi, k.key, k.value, QF_NO_LOCK);

	return Iterator(qfi, k.key);
}
